...
unsigned window_count = 0; // needed for registering application
...
/// <summary>
/// For releasing interfaces
/// </summary>
template<class T>
inline void safeRelease(T **obj_to_release) {
	if (*obj_to_release != nullptr) {
		(*obj_to_release)->Release();
		(*obj_to_release) = nullptr;
	}
}
...
gWindowHandle Window::createHandle(int x, int y, int width, int height) {
	if (window_count == 0) {
		HRESULT hr;
